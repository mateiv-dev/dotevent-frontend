    ...
    <div className="space-y-8 max-w-5xl mx-auto">
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl p-8 md:p-10 text-white shadow-xl shadow-blue-600/20 relative overflow-hidden">
        <div className="relative z-10">
          <h2 className="text-3xl md:text-4xl font-bold mb-3 tracking-tight">Welcome back, {currentUser.name}!</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <StatCard icon={<Calendar className="text-blue-600" size={24} />} label="Upcoming Events" value={events.length.toString()} />
        <StatCard icon={<CheckCircle2 className="text-green-600" size={24} />} label="Registered" value={registeredCount.toString()} />
        <StatCard icon={<Users className="text-purple-600" size={24} />} label="Total Students" value="2.4k" />
      </div>
        ...
        <Card className="p-8">
          ...
          {events.filter((e: Event) => e.isRegistered).length > 0 ? (
            <div className="space-y-4">
              {events.filter((e: Event) => e.isRegistered).slice(0, 3).map((event: Event) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-14 h-14 rounded-xl flex flex-col items-center justify-center text-sm font-bold shrink-0 transition-colors ${colorClasses}`}>
                      <span>{new Date(event.date).getDate()}</span>
                      <span className="uppercase text-[10px] font-extrabold opacity-70">{new Date(event.date).toLocaleString('default', { month: 'short' })}</span>
                    </div>
                    <div>
                      <h4 className="font-bold text-slate-900 line-clamp-1 text-lg group-hover:text-blue-700 transition-colors">{event.title}</h4>
                      <p className="text-sm text-slate-500 font-medium mt-0.5">{event.time} • {event.location}</p>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="text-center py-12 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
              <p className="text-slate-400 font-medium">No upcoming registrations.</p>
              <Button onClick={toEvents} variant="ghost" className="text-blue-600 hover:bg-transparent hover:underline p-0 h-auto mt-2">
                Browse events
              </Button>
            </div>
          )}
