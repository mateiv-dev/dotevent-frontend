...
export const Checkbox = React.forwardRef<HTMLInputElement, CheckboxProps>(
  ({
    ...
    return (
      <label className={`flex items-center gap-3 cursor-pointer ${containerClassName}`}>
        <div className="relative flex items-center">
          ...
          <div className={`
            ...
            ${className}
          `}>
            <Check size={14} className="text-white opacity-0 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transition-opacity" strokeWidth={3} />
          </div>
        </div>
        {label && (
